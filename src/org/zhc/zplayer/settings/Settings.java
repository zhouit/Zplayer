package org.zhc.zplayer.settings;
  ...
  TextField lyricLocation;
   ...
   public Settings(){
     ...
     Label title=LabelBuilder.create()
		 .translateY(5).text("选项设置").textFill(Color.WHITE)
		 .build();
	 ...
	 EventHandler<MouseEvent> aboutHandler=new EventHandler<MouseEvent>(){
	  ...
	  HBox head=HBoxBuilder.create()
		...
		.spacing(312).children(title,close)
		.build();
   ...
   public TabPane loadTab(){
	 ...
	 HBox lhbox=HBoxBuilder.create().padding(new Insets(0, 0, 0, 10))
		.spacing(5).alignment(Pos.CENTER_LEFT)
		.children(new Label("歌词文件保存路径:"),lyricLocation,ButtonBuilder.create()
			 .text("...").onAction(this).build())
		.build();
	 lyric.setContent(VBoxBuilder.create()
		.padding(new Insets(10, 0, 0, 20)).alignment(Pos.TOP_CENTER)
   ...
   Node createBottom(){
	 HBox bottom=HBoxBuilder.create()
	   ...
	   .children(ButtonBuilder.create().onAction(this)
		   .text("确定").prefHeight(27).prefWidth(68).id("button").build(),
	     ButtonBuilder.create().prefHeight(27).prefWidth(68).onAction(this)
		  .text("取消").id("button").build()).build();
   ...
   Node createCaption(String header){
	  return HBoxBuilder.create()
		.children(new Label(header),LineBuilder.create()
		    .startX(0).startY(0).endX(280).endY(0).translateY(6).translateX(12).build())
		.build();
    }
